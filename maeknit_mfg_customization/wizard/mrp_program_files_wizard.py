...
class MrpProgramFilesWizard(models.TransientModel):
    _name = 'mrp.program.files.wizard'
    _description = 'Program Files Viewer Wizard'

    workorder_id = fields.Many2one('mrp.workorder', string='Work Order', required=True)
    file_type = fields.Selection([
        ('program', 'Program Files'),
        ('instruction', 'Instruction Files')
    ], string='File Type', default='program', required=True)
    ...
    wizard_title = fields.Char(string='Wizard Title', compute='_compute_wizard_title')
    wizard_description = fields.Char(string='Wizard Description', compute='_compute_wizard_description')

    @api.depends('file_type')
    def _compute_wizard_title(self):
        for record in self:
            if record.file_type == 'instruction':
                record.wizard_title = 'Instruction Files'
            else:
                record.wizard_title = 'Program Files'
    
    @api.depends('file_type')
    def _compute_wizard_description(self):
        for record in self:
            if record.file_type == 'instruction':
                record.wizard_description = 'Select and preview instruction files for this operation'
            else:
                record.wizard_description = 'Select and preview program files for this operation'

    @api.depends('workorder_id', 'file_type')
    def _compute_available_attachments(self):
        for record in self:
            if record.workorder_id:
                if record.file_type == 'instruction':
                    attachment_ids = record.workorder_id.get_instruction_attachments(record.workorder_id.id)
                    logging.info(f"Found {len(attachment_ids)} instruction attachments for workorder {record.workorder_id.id}")
                else:
                    attachment_ids = record.workorder_id.get_program_attachments(record.workorder_id.id)
                    logging.info(f"Found {len(attachment_ids)} program attachments for workorder {record.workorder_id.id}")
    ...
    @api.model
    def open_program_files_wizard(self, workorder_id):
        wizard = self.create({'workorder_id': workorder_id})
        wizard._compute_available_attachments()
        return {
